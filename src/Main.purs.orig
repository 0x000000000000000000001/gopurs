        ...
        when (Array.length (Array.fromFoldable backendMod.foreign) > 0) do
          ...
          case ffiPathMb of
            ...
            Nothing -> do
              ...
              let dummyContent = "package " <> goPackageName <> "\n\nimport \"gopurs/output/gopurs_runtime\"\n\n" <> String.joinWith "\n" (map (\(Tuple ident _) -> "var _Gopurs_" <> CodeGen.capitalize (sanitizeName (unwrap ident)) <> " = gopurs_runtime.Func(func(_ gopurs_runtime.Value) gopurs_runtime.Value { panic(\"FFI not implemented: " <> unwrap ident <> "\"); return gopurs_runtime.Value{} })") (Map.toUnfoldable backendMod.foreign :: Array _))
              FS.writeTextFile UTF8 ("output/" <> modNameStr <> "/" <> goPackageName <> "_ffi.go") dummyContent
    }
    finalModules
