  ...
  in
    case expr of
      ...
      Lit (LitArray xs) ->
        let
          accXs = foldl
            ( \acc val ->
                let
                  resVal = translateExprImpl helpersRef (depth + 1) modNameStr recVars namedBound bound Nothing [] false acc.nextId val
                in
            ...
            Just index ->
              let
                accFinal = foldl
                  ( \acc arg ->
                      let
                        argRes = translateExprImpl helpersRef (depth + 1) modNameStr recVars namedBound bound Nothing [] false acc.nextId arg
                      in
      ...
      Accessor obj accessor ->
        let
          resObj = translateExprImpl helpersRef (depth + 1) modNameStr recVars namedBound bound Nothing [] false nextId obj
        in
          case accessor of
      ...
      Branch branches def ->
        let
          resDef = translateExprImpl helpersRef (depth + 1) modNameStr recVars namedBound bound Nothing loopCtx isTail nextId def
          ...
          buildIfs = foldl
            ( \acc (Pair condExpr bodyExpr) ->
                let
                  resCond = translateExprImpl helpersRef (depth + 1) modNameStr recVars namedBound bound Nothing [] false acc.nextId condExpr
      ...
      PrimOp op -> case op of
        Op1 op1 e ->
          let
            resE = translateExprImpl helpersRef (depth + 1) modNameStr recVars namedBound bound Nothing [] false nextId e
            goOp = case op1 of
