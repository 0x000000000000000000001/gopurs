...
const startHelper = code.indexOf('let parseFuncType = function(t) {');
const endHelper = code.indexOf('for (let idx = 0; idx < parsedArgs.length; idx++) {', startHelper);
const helpersCode = code.substring(startHelper, endHelper);

code = code.substring(0, startHelper) + code.substring(endHelper);

const insertPoint = code.indexOf('// 1. Generate Native Call_X proxy');
code = code.substring(0, insertPoint) + helpersCode + code.substring(insertPoint);
