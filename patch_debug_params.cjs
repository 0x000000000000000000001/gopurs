...
const replacementStr = `assigns = Array.zipWith (\\(Tuple param goT) argExpr -> GoMutate param (Debug.trace ("GoMutate assignment for " <> param <> ": from " <> goTypeToStr argExpr.exprType <> " to " <> goTypeToStr goT) \\_ -> coerceGoExpr modNameStr argExpr.expr argExpr.exprType goT)) paramsWithTypes argExprs`;
