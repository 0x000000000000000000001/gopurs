...
- [ ] *Action* : Empêcher la fuite de typage polymorphe lors de l'inlining.
  ...
  - [ ] **Baby Step 6.5 : Corriger la substitution pour les AST inlinés** : En fonction du diagnostic (ex: l'inliner PBO introduit des variables de type implicites, ou le compilateur PureScript génère des `t1` locaux), adapter `Monomorphize.purs` pour unifier ou propager correctement les types spécialisés jusqu'aux `LetRec` inlinés.
  - [ ] **Baby Step 6.6 : Validation finale** : Re-compiler `Test.Primes` et valider dans `Test_Primes.go` que la boucle interne `go` de `reverse` inlinée ne contient plus `gopurs_runtime.Value` (type strict `int64` attendu).
