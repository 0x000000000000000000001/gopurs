...
- [x] **1.5** Fixture étendue à 52 assertions : frontières `Let` avec calcul partagé, branche, `LetRec`, `App`, effet retournant une fonction, `UncurriedAbs` et `TypeApp`. Snapshot relu : opérations intermédiaires conservées dans la première fermeture unaire ; `Func` retournant `Func2` pour `UncurriedAbs`, deux `Func` pour `TypeApp`. Lecture et modification de `Ref` uniquement à l'exécution de l'effet, capture et réutilisation vérifiées. `./bin/test CurriedLambdas -c` et l'exécution du JavaScript PureScript passent ; aucune correction du compilateur nécessaire.
