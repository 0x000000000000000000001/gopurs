...
État au 21 septembre 2026 : **descriptions des types TAST décodées une seule fois dans PBO**, après correction de `forceEscape` et passage à 8 workers d'émission. Comparaison b8x isolée : chargement **31,492 → 25,435 s (−19,23 %)**, backend **180,180 → 176,312 s (−2,15 %)**. Validation par le build propre actuel `b -c` (natif par défaut, `-n` supprimé séparément du lanceur) : **181,220 s de backend**, commande **267,697 s**, sans erreur. Référence initiale du backend : 591,665 s.

**Dernière sous-étape, point 11 : décodage des tables de types accéléré.** Petit corpus : chargement natif −13,59 %, backend −4,76 % ; 741 410 types comparés à l'ancien décodeur, erreurs/cycles/partage conservés. Les 2 655 TAST et 2 959 Go b8x restent identiques, y compris dans le workflow final. L'écart de chargement avec JS subsiste ; le backend total du petit corpus est désormais proche (natif 9,067 s / JS 8,872 s). Les points 3, 4, 6, 7, 8, 10 et 11 restent ouverts.

## Points à traiter un par un, ultérieurement

**Prochaine sonde, point 11 : ventiler le coût natif restant de `decodeModule` après cette correction**, entre résolution des types et construction/décodage du reste de l'AST, avec temps et allocations. Le chargement reste environ 4,85 fois plus lent que JS sur le petit corpus ; remplacer le parseur JSON ou multiplier les workers n'est pas encore justifié. Le prochain gros chantier de parallélisme reste le point 4 : ordonnancement PBO entre modules avec transmission des directives conservée. Émission 8, préparation 2 retenues.
...
- [ ] **11. Écart JS/natif au chargement TAST.** Le natif utilise déjà `encoding/json.Unmarshal`. La sonde sur 304 modules attribue 3,096 s sur 3,555 s de chargement au décodage PBO du module, contre 0,00958 s au tri. **Descriptions des types décodées une seule fois et indices en attente conservés dans l'ordre** : petit corpus natif 3,4105 → 2,9470 s, b8x 31,492 → 25,435 s ; mêmes types, erreurs, cycles et Go final. La liste d'attente seule avait ralenti et a été écartée comme optimisation isolée. Prochaine étape : mesurer, après correction, les parts et allocations de la résolution des types et du reste du décodage AST ; l'écart avec JS n'est pas résorbé. Pas de cache entre builds ni perte d'information TAST.
...
## Point 11 — décodage des descriptions de types, intégré et mesuré
