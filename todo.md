...
- [x] *Action* : Aplatir les retours de structures de données éphémères (Maybe, Tuple, Either, State) aux frontières d'appels purs.
  ...
  - [x] **Baby Step 2.3** : Générer automatiquement une closure "pont" qui ré-emboîte (re-boxe) ces variables natives dans un vrai constructeur (ex: `Just`) *uniquement* lorsque la fonction est passée comme argument polymorphe de première classe (ex: passée à un `Array.map`).
  - [x] **Baby Step 2.4** : Mesurer sur un benchmark intensif (ex: 10 millions de `Map.lookup` ou une grosse boucle `StateT`) la chute vertigineuse de la pression sur le Garbage Collector et l'accélération d'exécution.AddNewsTopic
...
- [x] *Action* : Remplacer l'usage des dictionnaires dynamiques par des structures Go natives (ou un ShapeArray) lorsque le polymorphisme de rangée n'est pas requis (records fermés) ou suite à la monomorphisation.
  - [x] **Baby Step 3.1** : Identifier dans le TAST (via `ann.type` ou la passe de spécialisation) les sites d'instanciation et de manipulation où un record possède un type fermé et exact (sans variable de rangée `| r`).
  - [x] **Baby Step 3.2** : Modifier le CodeGen pour déclarer dynamiquement une `struct` Go native (ex: `type Record_a_Int_b_String struct { a int64; b string }`) associée à ce type exact.
...
- [x] *Action* : Exploiter le TAST pour inliner statiquement les méthodes de Type Classes lorsque le dictionnaire concret est connu, éliminant ainsi l'interface `gopurs_runtime.Value` du chemin critique.
  ...
  - [ ] **Baby Step 4.4** : Valider sur le benchmark `State Monad` une chute totale des allocations et un temps d'exécution passant à < 2 ms (niveau FFI Natif).
