...
Le programme PureScript du benchmark et le runtime sont inchangés : la passe enlève les closures intermédiaires, tout en conservant le million d'additions. Le test [`tests/passing/ThunkFusion.purs`](tests/passing/ThunkFusion.purs), lancé par `./bin/test ThunkFusion -c` avec le frontend TAST, couvre profondeurs et seeds variables, captures, ordre non commutatif, paramètres simultanés, collisions de noms Go, closures échappées, demandes conditionnelles et effets. Son snapshot Go vérifie aussi le code généré. Le typage général des closures (5.1–5.3), la mémoïsation de `Data.Lazy` et la simplification algébrique (5.5) restent des chantiers distincts.

**Périmètre sémantique :** les blocs locaux `LetRec` sont entièrement exclus de la fusion. Une annotation `Int` ne suffit pas à prouver qu'une valeur récursive est déjà initialisée ; cette barrière empêche d'avancer sa lecture et de dépendre du contrôle d'initialisation actuel du backend. Les cas `runRecursiveBinding` et `runRecursiveBody` de `tests/passing/ThunkFusion.purs` conservent de véritables scopes récursifs après optimisation ; le snapshot vérifie que les consommateurs placés dans ces scopes gardent leur producteur original, tandis que les consommateurs ordinaires utilisent la fonction optimisée.
