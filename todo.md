...
**Construction partagée des records JSON intégrée, 22 septembre — décodage −15,09 %, total −8,95 %.** Une boucle commune remplit le résultat final depuis les vrais callbacks de champ ; elle supprime les records intermédiaires, les coercitions répétées d’`IsSymbol` lors de l’insertion et les résultats récursifs de rangée. API publique Argonaut inchangée, repli sur les instances personnalisées. JSON général : **13,385 → 11,365 ms de décodage**, **16,599 → 15,113 ms combinés**, allocations décodage **19 714 272 → 16 556 992 octets (−16,02 %)**. Deux séries appariées regroupées, aucun run retiré. **TAST : aucun gain établi**, allocations exactement inchangées ; total **428,852 → 424,097 ms**, plages largement recoupées. README Go **15,11 / 424,10 ms**, JS **9,28 / 80,26 ms** ; JSON général reste à **×1,63** de JS. Compilateur natif final reconstruit/installé ; 174 tests compiler passent (2 skips), 8 tests runner, tests runtime/FFI avec race, fixture PureScript Go+JS et oracles 17 cas/12 modules passent. **Payloads et tableaux encore en `Value` ; aucun nouveau b8x.** [Rapport et archives](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-json-record-plans.md) · [Contrat](docs/json-record-plans.md). Le patch de l’overlay Argonaut sans dépôt Git est archivé avec le rapport.

**Sonde des callbacks partagés, 22 septembre — aucune intégration justifiée.** Trois variantes sur le vrai Go généré conservent les retours `Either` puis `Maybe (Either)` natifs entre les workers partagés. Dernière variante : décodage **13,096 → 13,156 ms**, allocations **19 714 272 → 18 513 488 octets (−6,09 %)**, 17 oracles identiques dans chaque processus. Deux paires par variante : aucun gain de temps démontré. Les payloads restent en `Value`, le lookup et le décodeur amont restent boxés, les insertions copient toujours les records ; cette coupe ne teste donc pas le modèle natif complet. Ne pas intégrer une seconde entrée de callback pour ces seules enveloppes. README inchangé : Go **17,00 / 422,27 ms**, JS **9,30 / 76,80 ms** combinés. [Rapport, limites et mesures](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-shared-native-callbacks.md).

**Chantier parsing JSON direct clos pour les applications, 22 septembre — parsing −65 % sur les deux corpus.** FFI `gopurs-argonaut-core` : grammaire JSON et échappements natifs, chaînes possédant leur stockage, erreurs exactes via repli stdlib. Campagne appariée : JSON général **22,766 → 16,999 ms combinés (−25,33 %)**, parsing **7,245 → 2,529 ms** ; TAST **494,614 → 422,267 ms combinés (−14,63 %)**, parsing **76,631 → 26,764 ms**. Allocations parsing **−23,09 % / −18,12 %**, allocations décodage inchangées. README Go **17,00 / 422,27 ms**, JS **9,30 / 76,80 ms** ; JSON général à **×1,83** de JS, TAST à **×5,50**. Sept tests Go avec/sans `-race`, 5 357 cas générés/mutés + 106 dirigés + 323 JS, suite Argonaut et oracles 17 cas JSON/12 modules TAST passent. Neuf formes synthétiques gagnent, y compris les échappements après correction de leur régression initiale. **Pas de nouveau b8x ni de rebuild du compilateur auto-hébergé** : les applications sont régénérées avec la FFI finale, son effet sur le chargement du compilateur reste à valider après reconstruction. Le prochain chantier reste le décodage/ABI partagé, pas une nouvelle promesse de parsing. [Rapport et archive](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-native-json-parser.md) · [Contrat du parseur](../gopurs-argonaut-core/docs/native-parser.md).

**Traversées fusionnées et callbacks natifs intégrés le 22 septembre — campagne appariée finale : JSON général, décodage 22,752 → 14,622 ms (−35,73 %), combiné 31,791 → 24,938 ms (−21,56 %) ; TAST, décodage 428,356 → 384,991 ms (−10,12 %), combiné 531,295 → 498,367 ms (−6,20 %).** Allocations de décodage −35,20 % / −7,13 %, binaires stables. Fusion Object/Either sans copies successives de map ; callbacks littéraux Array natifs ; 19 sites non indexés TAST couverts ; erreurs de champs PBO différées. Tableaux finaux encore en `[]Value`, ABI des callbacks stockés dans les dictionnaires toujours ouverte. README : Go **24,94 / 498,37 ms**, JS **9,18 / 83,42 ms** ; les anciennes cellules étaient 27,87 / 505,42 ms Go, d'une autre session. **171 tests passent, 2 skips ; 4 fixtures natives ; oracles 17 cas JSON et 12 modules TAST dans chaque processus.** Pas de nouveau b8x complet. [Rapport et mesures](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-fused-json-traversals.md) · [Contrat](docs/json-traversals.md).

**Erreurs différées dans `argonaut-codecs` intégrées le 22 septembre — JSON général : décodage 31,58 → 27,87 ms cumulés (−11,7 % sur cette étape, −17,5 % pour `caseJson*`), allocations décodage 41,65 → ~29 Mio cumulées.** Fork local `gopurs-argonaut-codecs` : `AtKey`/`AtIndex` ne sont construits que sur le chemin d'échec (`gDecodeJsonCons`, `decodeArray`, `getField`, `getFieldOptional`, `getFieldOptional'`). Statique : sites `AtKey` 9 → 1. TAST : neutre (−0,35 %). JS : neutre/positif (−3,7 % / −4,4 %). Cellule README JSON Go 27,87 ms (contrôle même-session 38,87 → 27,87, protocole officiel). [Rapport](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-argonaut-error-paths.md).

**FFI dédiées `caseJson*` intégrées le 22 septembre — JSON général : combiné 37,19 → 31,58 ms (−15,1 %), décodage 27,61 → 22,91 ms (−17,0 %), allocations décodage 41,65 → 33,81 Mio (−18,8 %) ; TAST : combiné 560,75 → 505,42 ms (−9,9 %), décodage −10,0 %, allocations décodage 488,14 → 454,56 Mio.** Les six helpers `caseJsonNull/Boolean/Number/String/Array/Object` passaient par `caseJson` avec 5-6 callbacks `const d` : six closures Go construites par appel avant que `CaseJsonImpl` n'en applique une. Chacun a maintenant sa FFI dédiée, de sémantique identique (y compris le repli `TypeAny`/constructeur vers la branche objet). `Data_Argonaut_Core.go` passe de 114 à 24 `Func(` (−79 %). Validation : oracles 17 cas + 12 modules sur les deux backends, `modtest argonaut-core` 10/10, 170 tests node (0 échec). Campagne appariée : décodage JSON −17,5 %, TAST −8,6 %. [Rapport et archive](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-casejson-ffi.md).

**Chaînes packées intégrées le 22 septembre — décodage seul : JSON général 30,917 → 27,581 ms (−10,79 %), TAST 450,455 → 413,480 ms (−8,21 %) ; combinés −4,61 % / −4,36 % ; allocations −2,68 / −23,13 Mio par corpus.** `TypeString` stocke désormais la longueur dans `IntVal` et les données dans `UnsafePtr` (`unsafe.StringData`) ; le runtime et 13 sites FFI passent par `StrValue`, et le runtime embarqué est régénéré. Les oracles (17 cas JSON, 12 modules TAST) sont identiques dans tous les processus, deux campagnes appariées reproduisent la direction, et les contrôles passent : 168/170 tests node (2 skips, 0 échec), `argonaut-core`, `foreign`, `foreign-object`, `node-buffer`, `prelude` et `strings-extra` verts, 14 cas core exacts. L'échec `gopurs-strings` sur les surrogates isolés est **préexistant** (reproduit à l'identique avec le bundle baseline). Cellules README Go mises à jour (37,68 / 524,28 ms). Pas de nouveau b8x complet ; la rétention des sous-chaînes est documentée. [Mesures, contrats et limites](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-packed-strings.md) · [Contrat runtime](docs/packed-strings.md).

**Bilan intégré du 22 septembre — JSON réel : 45.321 → 37.187 ms (−17.95 %), TAST : 565.532 → 560.745 ms (−0.85 %).** Go régénéré par le compilateur natif final ; cellules README JS également remesurées (9.638 / 77.022 ms). Fusion Array/Either, applications immédiates et conservation des sommes natives aux frontières Value intégrées. Les allocations combinées baissent de 19.84 % / 1.46 %. Le gain de débit TAST reste **non concluant** (plages recoupées) ; seule sa petite baisse d’allocations est établie. La conservation des sommes seule ne démontre presque aucun gain JSON ; les payloads/callbacks natifs restent le prochain verrou. **Les 8–9 ms manuelles ne sont pas atteintes.** [Mesures, ablations et limites](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-generated-json-optimizations.md). Pas de nouveau b8x complet.

**Première brique native générée, intégrée le 22 septembre :** un lecteur polymorphe de rangée ouverte peut maintenant recevoir les champs primitifs connus en struct native, avec **un seul corps partagé** entre les formes de records. Sur du vrai PureScript généré : pipeline substantiel **52,61–133,20 → 2,92–3,24 ns**, allocations **176–448 → 0 octet** ; petit cas déjà inliné inchangé. Ce sont des microbenchmarks de frontière d'appel, **pas de nouveaux chiffres JSON**. La projection est limitée aux lectures prouvées et au retour scalaire ; les records retournés et résultats `Either` restent le prochain jalon architectural. Détails ci-dessous.

Étape précédente du 22 septembre 2026 : **records compacts et traversée FFI Value intégrés**. Diagnostic applicatif JSON général : **61,042 → 45,816 ms (−24,94 %)**, décodage seul **−27,14 %**, allocations combinées **−24,84 %**. JS actuel **8,647 ms**, écart restant **×5,30**. Le TAST profite peu de ces changements : **582,022 → 567,609 ms (−2,48 % observés, plages qui se recoupent)**. Baselines README précédentes : Go **57,43 / 573,63 ms** pour JSON général / TAST. Aucun nouveau b8x complet. Détail et suites générales au point 12 ci-dessous.

**Comparaison historique des pistes, 22 septembre :** 16 variantes × 3 processus, puis sonde de mises à jour immuables. Sur le **vrai Go généré modifié en scratch**, fusionner le décodage des tableaux donne **45,364 → 35,193 ms (−22,42 %)** ; avec bêta-réduction **34,709 ms (−23,49 %)**, JS témoin **9,051 ms**. Dans le moteur manuel partagé, modifier seulement l'accès au dictionnaire gagne **6,02 %** ; enlever aussi les enveloppes `Value` aux appels/résultats accélère le décodage de **×4,35**. Les prototypes natifs complets restent vers **8,3–8,5 ms** avec parsing. **Ces chiffres appartiennent aux prototypes de cette campagne.** La fusion des tableaux et la réduction des applications immédiates sont désormais intégrées ; leur mesure réelle, distincte des prototypes, est présentée dans le bilan courant. Interfaces et descripteurs restent des outils complémentaires ; les chaînes naïves de wrappers se dégradent fortement. Détails dans la première section du point 12.
...
**Priorité restante : profiler la construction du TAST (point 11), puis choisir sa prochaine cible mesurée.** La construction partagée des records JSON du point 12 est maintenant intégrée, mais ne réduit aucune allocation du TAST. Son décodeur propre, la table de types et la boucle ST de `CoreFn.Json.decodeArray` doivent être départagés par un profil renouvelé. Côté JSON général, la prochaine étape architecturale reste de conserver arguments et payloads typés entre décodeur de champ, dictionnaire, worker et stockage final : les buffers sont encore en `Value`. Ne pas répéter la sonde des seules enveloppes ni présenter la construction unique comme une ABI native complète. Les 8–9 ms des moteurs manuels restent un potentiel architectural, pas un résultat du compilateur.

**Point 11 toujours ouvert :** la FFI de traversée est allégée, mais le TAST ne gagne que modestement. Reprofiler ce décodeur avant sa prochaine optimisation propre ; `resolveArgs` et les applications à 6–10 arguments sont déjà corrigés. Le parseur natif d'Argonaut est maintenant remplacé par un chemin direct compatible, validé sur JSON/TAST ; ce sous-chantier est clos pour les applications régénérées. Le compilateur natif est désormais reconstruit avec cette FFI et le plan de records ; mesurer son chargement puis le vrai b8x complet reste à faire (point 7). Le prochain gros chantier de parallélisme reste le point 4 : ordonnancement PBO entre modules avec transmission des directives conservée. Défauts natifs : chargement 8, émission 8, préparation 2 ; chargement JS 1.
...
- [ ] **11. Écart JS/natif au chargement TAST.** Descriptions des types décodées une seule fois dans PBO, puis correction générale de `OpArrayIndex` dans gopurs : convertir seulement l'élément sélectionné évite les copies répétées du tableau d'indices en attente. Cette ligne représentait **3,15 Go d'allocations** dans le profil Aff. À 1 worker : chargement **2,9315 → 2,4540 s** ; avec le nouveau défaut natif à 8 : **1,3380 s**. JS final : **0,5855 s**. Backend natif **7,8185 s / JS 6,9665 s** ; tests Aff complets et sorties JS/native comparées. Chargement b8x −35,64 % entre 1 et 8 workers, mémoire de phase +0,650 Gio ; validation du backend b8x complet à refaire. La FFI Argonaut dispose maintenant du parseur direct validé ci-dessus ; `encoding/json.Unmarshal` reste le repli sur les entrées rejetées par le chemin rapide. Le compilateur natif est reconstruit ; mesurer son chargement avant d'attribuer le gain applicatif à b8x. Aucun cache entre builds ni perte d'information TAST.

- [ ] **12. Performances générales du code Go, révélées par JSON Decoding.** Records compacts, lecteurs polymorphes natifs partagés, réduction des applications immédiates et fusion Array/Either intégrés. La fusion couvre maintenant les formes non indexées utilisées par le TAST et Object/Either ; les callbacks littéraux peuvent conserver leur résultat natif jusqu'à la boucle. Les helpers de champs PBO construisent leurs erreurs seulement sur le chemin d'échec. La construction partagée des records Argonaut est intégrée (−15,09 % décodage JSON), avec une matérialisation finale unique mais des slots `Value`. Restent **(a)** callbacks stockés dans les dictionnaires/getters avec arguments et succès natifs à travers les workers ; **(b)** records retournés et rangées complètes sans perte des champs supplémentaires ; **(c)** tableaux finaux encore en `[]Value` et allocations ADT qui échappent réellement ; **(d)** interfaces/descripteurs plats selon les besoins, sans chaînes de wrappers. Reprofiler le TAST, notamment sa boucle ST de `CoreFn.Json.decodeArray`, que la fusion de `traverse` ne traite pas. Garder immutabilité, ordre, première erreur, captures, appels partiels et ABI FFI ; mesurer temps, allocations et taille. La sonde locale `decodeMaybe` seule ne gagne aucune allocation. Ne pas présenter les 8–9 ms manuelles comme acquis ; b8x complet à revalider au point 7.
...
## Point 12 — retours natifs de callbacks partagés, sonde non intégrée du 22 septembre 2026

Le profil renouvelé porte sur le décodage JSON général pré-parsé, après le parseur direct. `Apply2` cumule **99,82 % des octets alloués**, mais sa part propre est **1,08 %** ; sur les piles CPU du benchmark, sa part propre est **0,92 %**. Ces cumuls incluent les décodeurs appelés et ne mesurent pas un coût de dispatch récupérable. Les allocations propres comprennent `Just` **8,09 %**, `Right` **7,22 %**, les constructions `RecordDict1..5` **15,07 %**, la coercition du dictionnaire `IsSymbol` **7,03 %** et les adaptateurs FFI du lookup **7,81 %**. Certaines allocations matérialisent les résultats finaux et ne sont pas toutes supprimables.

Trois expériences sur le Go généré figé, sans schéma manuel ni worker dupliqué par layout :

| Coupe testée | Décodage témoin → sonde (ms) | Octets alloués témoin → sonde |
|---|---:|---:|
| Retour natif du worker récursif `gDecodeJsonCons` | 13,130 → 13,155 | 19 714 272 → 19 221 592 |
| Avec retours `Maybe (Either)` des callbacks de champ | 14,318 → 15,266 | 19 714 272 → 18 513 488 |
| Même coupe, retour `Either` compact 32 plutôt que 56 octets | 13,096 → 13,156 | 19 714 272 → 18 513 488 |

Deux paires par variante, ordre témoin/sonde/sonde/témoin, deux chauffes et cinq échantillons par phase ; médiane des minima des processus, 1 P/GOGC=100/PGO désactivé. Oracles 17 cas identiques dans tous les processus. La campagne intermédiaire montre une variabilité jusque dans le parsing inchangé ; elle ne prouve pas une régression intrinsèque. La dernière variante ne démontre toujours aucun gain de débit malgré **−6,09 % d'octets**. Aucun nouveau TAST/b8x ni contrôle JS : le README officiel reste la référence historique, ses cellules ne changent pas.

**Décision : ne pas intégrer cette coupe.** Le lookup produit encore un `Maybe` boxé, le décodeur de champ un `Either` boxé, et les payloads/records/tableaux restent en `Value`. Les retours natifs enlèvent des enveloppes à deux frontières ; ils ne conservent pas encore le résultat natif depuis sa production jusqu'au stockage final. Le modèle complet reste ouvert, sans gain promis. Les scripts, résultats bruts et provenance sont archivés dans le [rapport](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-shared-native-callbacks.md).

## Point 12 — premier worker partagé natif, intégré le 22 septembre 2026

**Vrai code PureScript → TAST → gopurs → Go**, sans réécriture manuelle du worker Go. Un lecteur `forall r. { id :: Int, name :: String, active :: Boolean | r } -> Int` reçoit une struct native des champs connus, avec retour scalaire natif. Les appelants projettent ces champs depuis leurs records plus larges ; le record source est évalué une fois et reste complet chez l'appelant. **Projection de champs**, pas encore interface native préservant une rangée entière.

Deux gardes : preuve TAST avant exclusion de la monomorphisation, puis vérification des usages sur l'IR TCO final. Seulement lectures connues, champs primitifs et retour scalaire ; transfert, retour, mutation, capture du record ou types inconnus gardent le chemin ordinaire. Les signatures publiées sont utilisées par les déclarations et appels. Les wrappers `Value` restent disponibles pour appels dynamiques/partiels. Si la preuve finale échoue, le repli est correct mais ne restaure pas les spécialisations retirées : risque de régression de performance à surveiller.

| Pipeline généré substantiel | Avant (ns) | Après (ns) | Octets avant/après | Allocations avant/après |
|---|---:|---:|---:|---:|
| Record 4 champs | 52,610 | **2,921** | 176 / **0** | 2 / **0** |
| Record 5 champs, record imbriqué | 82,120 | **3,045** | 288 / **0** | 4 / **0** |
| Record 6 champs, tableau supplémentaire | 133,200 | **3,238** | 448 / **0** | 8 / **0** |

**Contrôle supplémentaire avec les trois champs variables à chaque appel : 56,300 → 3,102 ns (×18,15)**, 176 octets/2 allocations → 0/0. Fonction `runFields` également compilée depuis PureScript, 1 024 combinaisons de valeurs vérifiées par variante, 3 processus alternés de 100 ms. Le gain ne dépend donc pas seulement des noms/flags constants des premiers producteurs.

**×18–41 sur ces pipelines**, incluant construction et appel. Go peut aussi simplifier les valeurs de champs connues ; ce ratio n'isole pas le coût d'un getter. Petit cas de contrôle déjà inliné par PBO : **1,816/1,716/2,606 → 1,825/1,703/2,591 ns**, allocations nulles des deux côtés, aucune différence significative. Les clones ne sont donc pas artificiellement maintenus hors ligne dans le témoin.

Un corps de worker **dans le Go généré**, contre l'original et trois clones auparavant. Le compilateur Go conserve son droit d'inliner. Taille du binaire applicatif substantiel **3 523 346 → 3 523 202 octets** ; petit cas **3 506 626 → 3 523 138** (+0,47 %). Pas de promesse de taille constante sur tout programme.

**Validation :** 18 tests ciblés passent ; 4 fixtures compilées/exécutées avec le gopurs natif reconstruit et le vrai fork TAST ; 768 cas entrées/layouts par exécutable mesuré. Immutabilité, champs supplémentaires, cas d'échappement et appels dynamiques/partiels contrôlés. Bootstrap natif de 454 modules réussi. Trois processus par variante en ordre alterné, 200 ms par cellule, 1 P/GOGC=100/PGO off, aucune autre activité de la tâche pendant les timings. Baselines README JSON **45,82 / 8,65 ms** et TAST **567,61 / 79,06 ms** inchangées ; **aucun nouveau chiffre JSON/TAST/b8x**.

**Suite architecturale : résultat record natif et frontière `Either`, puis mesure sur le décodeur réel.** La première projection scalaire ne suffit pas à généraliser le prototype manuel de 8–9 ms.

[Rapport et mesures](/Users/0x1/Documents/htdocs/altbak.pub-gopurs/docs/benchmark-results/2026-09-22-native-record-workers.md) · [Contrat du compilateur](/Users/0x1/Documents/htdocs/gopurs/gopurs/docs/native-record-workers.md).

## Point 12 — sélection des axes après comparaison, 22 septembre 2026

Campagne historique de sélection ; la première intégration et la priorité actuelle sont décrites dans la section précédente.
