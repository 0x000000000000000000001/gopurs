...
- **Action** : Étendre le parcours du monomorphiseur (dans `purescript-backend-optimizer`) pour qu'il inspecte et spécialise les déclarations `LetRec` locales.
  - [x] **Baby Step 5.1 : Traversée des LetRec dans le Monomorphiseur** : Ajouter la visite récursive des expressions de type `ExprLet` et `ExprLetRec` dans `Monomorphize.purs` de PBO pour collecter les signatures d'appel des fonctions locales.
  - [x] **Baby Step 5.2 : Clonage et Substitution locale** : Si une fonction locale polymorphe (ex: `go`) est appelée avec un type spécifique (ex: `Int`), générer une version spécialisée (`go_Int`) dans les `Binders` du `LetRec` et substituer les appels internes.
  - [x] **Baby Step 5.3 : Validation des types TypesApp locaux** : S'assurer que le passage des variables de type (via `TypeApp`) aux fonctions locales est correctement résolu et effacé une fois la fonction spécialisée.
  - [x] **Baby Step 5.4 : Unification des appels inlinés** : Remplacer l'extraction de `TypeApp` dans `collectLocalExpr` par une unification de types dynamiques (`unify`) entre le type formel de la fonction (`ann.type`) et le type concret de ses arguments réels (provenant de `spine`).
  - [x] **Baby Step 5.5 : Utilisation de la substitution dynamique** : Mettre à jour la logique de clonage dans `ExprLet` pour utiliser directement le dictionnaire de substitution (`Map String ExprType`) déduit par `unify`, permettant ainsi de capturer les variables des fonctions inlinées (ex: `a1 = Int` pour `reverse`).
  - [ ] **Baby Step 5.6 : Validation sur Test.Primes** : Compiler `Test.Primes` et vérifier dans le Go généré que `go` (de `filter`) et `go` (de `reverse`) sont devenus strictement monomorphes (`int64`), sans aucun appel `Rebox_...`. Comparer les performances avec le benchmark de référence (viser moins de 10 µs).
...
### 6. Typage strict des boucles internes inlinées
Le code généré pour la fonction globale spécialisée `filter` est correct, mais la boucle récursive `go` de la fonction `reverse` inlinée par PBO (via `Optimizer.purs`) conserve son type générique d'origine et force un reboxing (`gopurs_runtime.Value`).

- [ ] *Action* : Empêcher la fuite de typage polymorphe lors de l'inlining.
  - [x] **Baby Step 6.1** : Investigate `Optimizer.purs` to see if/how type annotations on AST nodes are preserved during inlining.
  - [x] **Baby Step 6.2** : Fix the inliner to either substitute type variables appropriately or ensure `Monomorphize.purs` can reach and strip `ForAll` bindings from inlined foreign module nodes.
  - [ ] **Baby Step 6.3** : Re-run benchmarks (`Prime Sieve`) to confirm `go__go` loops inside inlined functions (e.g., `reverse`) compile to strict Go structs rather than falling back to `gopurs_runtime.Value` interfaces.
  - [x] **Baby Step 6.4 : Isoler le problème des variables orphelines** : Ajouter des traces ou scripts de dump pour inspecter les variables de type (`ann.type`) de la boucle `go` inlinée de `reverse` au sein de `filter`, afin de prouver pourquoi ces variables ne matchent pas avec la substitution de `filter` (`{ "a" -> Int }`).
  - [ ] **Baby Step 6.5 : Corriger la substitution pour les AST inlinés** : 
    - **Diagnostic établi** : L'optimiseur PBO (`Semantics.purs`) inline physiquement la boucle `go` de `reverse` au sein de `filter`. En faisant cela, il conserve les variables de type d'origine de `reverse` (ex: `a`). Lorsque `filter` est spécialisé via `rewriteExpr` (ex: avec la substitution `b -> int64`), la variable `a` est ignorée car inconnue de la substitution. Lors de l'évaluation par `collectLocalExpr`, `unifySpine` unifie `List a` avec `List a`, ce qui produit la substitution inachevée `{ "a" -> TypeVar "a" }`. En l'absence de type concret, le générateur de hash (`mangleType`) utilise son fallback `defaultToAny` et génère un type `Any` (`gopurs_runtime.Value`).
    - **Action requise** : Modifier `Monomorphize.purs` (probablement au niveau de `collectLocalExpr` ou `rewriteExpr`) pour forcer l'unification ou propager les types concrets aux variables orphelines des `LetRec` inlinés, afin que la variable `a` soit correctement résolue en `int64`.
  - [ ] **Baby Step 6.6 : Validation finale** : Re-compiler `Test.Primes` et valider dans `Test_Primes.go` que la boucle interne `go` de `reverse` inlinée ne contient plus `gopurs_runtime.Value` (type strict `int64` attendu). In fine, faire bin/go/run -c, dans altbak.pub, doit passer.
