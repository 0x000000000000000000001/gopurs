...
État au 21 septembre 2026 : **boxage des pointeurs ADT conservé dans l’AST Go**, après les optimisations précédentes. Dernier vrai `b -c -n` : backend **219,560 s (3 min 40)**, commande entière **295,235 s (4 min 55)**, sans erreur. La référence juste avant était 229,134 s / 301,760 s ; référence initiale du backend : 591,665 s. Les essais sans gain établi restent écartés.

**Dernière sous-étape, point 8 : optimisation/émission 146,288 → 135,805 s (−7,17 %), backend −4,18 %, allocations 394,78 → 382,36 Gio (−3,15 %).** Les 22 tests passent ; 2 655 TAST et 2 959 Go identiques. Le petit corpus alterné confirme un gain modeste (backend −2,55 %). La sonde Str/StrVal n’apportait rien et est écartée ; le boxage structuré fait passer le scanner de **54,77 à 31,62 Gio**. Prochaine priorité : les recherches d’implémentations et la construction des expressions PBO ; mesurer les répétitions avant un nouveau cache. Le parcours des imports structurés coûte davantage, à prendre en compte avant extension aux records/tableaux. Les points 3, 4, 6, 7, 8 et 10 restent ouverts ; point 5 validé sur le vrai build Go et les tests JS/PHP.
