...
État au 21 septembre 2026 : optimisations précédentes et **cache des contributions de collecte transitive** intégrés. Dernier backend b8x : **223,904 s (3 min 44)**, contre 591,665 s au départ (évolution historique **−62,16 %, 6 min 08 gagnées**). `b -c -n` entier : **299,516 s (environ 5 min)**. Les essais sans gain établi restent écartés.

**Dernière sous-étape, point 3 : préparation 82,013 → 45,096 s (−45,01 %), backend 268,011 → 223,904 s (−16,46 %), allocations 458,57 → 395,64 Gio (−13,72 %).** 59 tests ciblés passent ; 2 655 TAST et 2 959 Go strictement identiques. Un nouveau run complet et petit corpus alterné : préparation améliorée dans les deux cas, gain total établi seulement sur b8x. Les étapes finales du point 5 sont corrigées pour Go/PHP/JS : sourcemaps confiées au build Spago, configuration JS autonome sans Docker et erreurs propagées. 12 tests et un mini-build JS réel avec vérification PHP passent ; aucun nouveau rebuild complet de b8x. Prochaine priorité : les coûts restants de l’optimisation/émission (point 8, 145 s), puis les calculs parallélisables du point 4. Les points 3, 4, 6, 7, 8 et 10 restent ouverts.
