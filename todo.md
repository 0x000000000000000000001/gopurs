...
État au 21 septembre 2026 : **collecte transitive parallèle intégrée**, avec deux workers Aff natifs par défaut. Dernier vrai `b -c -n` : backend **196,494 s (3 min 16)**, commande entière **277,148 s (4 min 37)**, sans erreur. Référence immédiatement précédente : 207,831 s / 288,509 s ; référence initiale du backend : 591,665 s. Les essais sans gain établi restent écartés.

**Dernière sous-étape, point 4 : préparation 45,706 → 37,602 s (−17,73 %), backend −5,45 %.** 17 tests JS et 3 tests Go sous `-race` passent ; 2 655 TAST et 2 959 Go identiques. Allocations **+0,33 %**, pic RSS **5,911 → 6,067 Gio (+2,63 %)**. Un seul nouveau run b8x ; les variations de chargement/émission ne sont pas attribuées à ce changement. Sur le petit corpus alterné, préparation −6,91 %, sans gain global établi. La sonde préalable `Semantics.build` ne justifie pas un cache général par identité ; les recherches locales et autres allocations PBO restent à examiner. Les points 3, 4, 6, 7, 8 et 10 restent ouverts ; point 5 validé sur le vrai build Go et les tests JS/PHP.
