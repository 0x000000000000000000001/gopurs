...
**Priorité : point 12, poursuivre les signatures natives des décodeurs partagés.** Fusion Array/Either et applications immédiatement consommées désormais traitées. Le prochain vrai verrou est le **callback natif avec payload de succès typé**, conservé entre dictionnaire, worker et boucle de traversée : garder seulement la structure extérieure de `Either` laisse ses champs en `Value`. Préserver les layouts déjà plus précis. Prouver ce passage depuis PureScript sur un décodeur partagé, avec adaptateur aux appels dynamiques/FFI, puis mesurer JSON général et TAST. La projection de rangée actuelle reste limitée aux lecteurs scalaires ; retour/mise à jour de la rangée complète ouverts. Les 8–9 ms des moteurs manuels restent un potentiel architectural, pas un résultat du compilateur.
...
- [ ] **12. Performances générales du code Go, révélées par JSON Decoding.** Records compacts et lecteurs polymorphes natifs partagés intégrés. **Fusion Array/Either et réduction générale des applications immédiates intégrées**, validées sur le vrai code généré ; résultats complets dans le bilan courant ci-dessous. Les enveloppes natives `Maybe`/`Either`/`Tuple` et leur lecture directe sont traitées, mais les payloads restent en `Value`. Restent **(a)** callbacks/dictionnaires avec arguments et succès natifs jusque dans la boucle ; **(b)** records retournés et rangées complètes sans perte des champs supplémentaires ; **(c)** suppression des allocations ADT qui échappent réellement, mesurée à travers les appels ; **(d)** interfaces/descripteurs plats selon les besoins, sans chaînes de wrappers. Garder immutabilité, ordre, première erreur, captures, appels partiels et ABI FFI ; mesurer temps, allocations et taille. La sonde locale `decodeMaybe` seule ne gagne aucune allocation. Ne pas présenter les 8–9 ms manuelles comme acquis ; b8x complet à revalider au point 7.
