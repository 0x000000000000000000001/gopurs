...
État au 21 septembre 2026 : **contention de `forceEscape` supprimée**, durée de vie des closures validée sous GC et `-race`. Contrôle rapproché sur les mêmes TAST b8x : backend **210,202 → 189,835 s (−9,69 %)**. Dernier vrai `b -c -n` profilé : backend **202,215 s**, commande entière **290,815 s (4 min 51)**, sans erreur ; le profil précédent donnait 196,494 s / 277,148 s. Ces deux protocoles restent distingués. Référence initiale du backend : 591,665 s.

**Dernière sous-étape, point 4 : optimisation/émission b8x 134,846 → 117,677 s (−12,73 %) dans le contrôle rapproché**, CPU −6,42 %, pic RSS +1,51 %. Petit corpus alterné : backend −5,41 %. Trois tests Go et 66 sous-cas passent en mode normal et sous `-race`. Les 2 655 TAST et les 2 958 Go applicatifs sont identiques ; seul le runtime corrigé diffère parmi les 2 959 Go. Le profil complet réduit les allocations de 3,32 Gio, mais sa durée totale augmente : aucun gain fixe sur toute la commande n’est promis. Les points 3, 4, 6, 7, 8 et 10 restent ouverts ; point 5 validé sur Go et les tests JS/PHP.

## Points à traiter un par un, ultérieurement

**Prochaine priorité, point 4 : réévaluer les nombres de workers avec le runtime corrigé**, puis reprendre si pertinent l’ordonnancement PBO entre modules, en conservant la transmission des directives. La suppression du chemin partagé de `forceEscape` est désormais intégrée et validée ; les anciens essais de montée en concurrence doivent être interprétés avec cette limite historique.
