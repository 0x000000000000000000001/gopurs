...
**Priorité : point 12, poursuivre les signatures natives des décodeurs partagés.** Les callbacks littéraux directement consommés par Array/Either gardent maintenant leur signature et leur résultat natifs ; la fusion couvre aussi les traversées non indexées du TAST et Object/Either. La frontière dynamique reste ouverte : callbacks stockés dans les dictionnaires/getters, arguments et payloads transmis entre workers, puis tableau final encore en `[]Value`. Prochaine preuve courte : conserver ces représentations à travers un callback partagé réellement utilisé, avec adaptateurs pour les appels génériques/FFI, et mesurer JSON général et TAST. Les records natifs en résultat sont possibles localement ; la conservation des rangées complètes à travers les workers partagés reste à démontrer. Les 8–9 ms des moteurs manuels restent un potentiel architectural, pas un résultat du compilateur.
