...
État au 21 septembre 2026 : **cache des recherches externes PBO intégré**, après le boxage structuré des pointeurs ADT. Dernier vrai `b -c -n` : backend **207,831 s (3 min 28)**, commande entière **288,509 s (4 min 49)**, sans erreur. Référence immédiatement précédente : 219,560 s / 295,235 s ; référence initiale du backend : 591,665 s. Les essais sans gain établi restent écartés.

**Dernière sous-étape, point 8 : optimisation/émission 135,805 → 128,640 s (−5,28 %), backend −5,34 %, allocations 382,36 → 374,42 Gio (−2,08 %).** 31 tests JS et 10 tests Go sous `-race` passent ; 2 655 TAST et 2 959 Go identiques. Le petit corpus alterné montre un gain plus modeste (backend −1,70 %). La sonde trouve 93,16 % de répétitions parmi les recherches externes : cache FIFO de 512 paires par module, avec priorité locale préservée. Un seul nouveau run b8x ; les variations de chargement/préparation ne sont pas attribuées au cache. Prochaine priorité : construction d’expressions et recherches locales PBO encore répétées. Les points 3, 4, 6, 7, 8 et 10 restent ouverts ; point 5 validé sur le vrai build Go et les tests JS/PHP.
...
- [ ] **8. Coût interne des passes PBO et de l’émission Go.** `Array.any` générique et cache FIFO de 512 instanciations par module intégrés ; collecte transitive accélérée au point 3. **Boxage des pointeurs ADT désormais structuré**, texte Go et imports préservés : backend **229,134 → 219,560 s**, scanner **54,77 → 31,62 Gio**, allocations totales **−12,42 Gio** ; 22 tests et sorties b8x identiques. La suppression isolée de Str/StrVal est écartée (aucune allocation économisée). **Cache externe PBO maintenant intégré** : backend **219,560 → 207,831 s**, allocations **−7,94 Gio** ; 41 tests et sorties b8x identiques. `lookupImplementation` passe de **22,42 à 14,09 Gio**. Restent la construction d’expressions `Semantics.build` (**46,84 Gio**) et `makeExternEvalSpine` (**37,49 Gio**, dont recherches locales encore actives). Distinguer leurs répétitions et dépendances avant une nouvelle réutilisation. Pour poursuivre l’AST structuré sur records/tableaux, surveiller les imports : `exprImports` coûte **10,48 Gio** dans le dernier profil. Coûts cumulés, à ne pas additionner.
