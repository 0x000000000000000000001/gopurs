...
**Priorité : point 12, conserver les données natives sur une chaîne complète du décodeur.** La sonde des retours `Either`/`Maybe (Either)` partagés est désormais mesurée et insuffisante seule (section ci-dessous). Prochaine preuve différente : garder le résultat du décodeur de champ jusqu'à sa destination dans le record, puis le tableau, sans reboxage intermédiaire ni construction successive des records. Les types TAST doivent permettre cette conservation avec un worker partagé et un repli générique/FFI ; cette faisabilité reste à démontrer sur le vrai décodeur. Mesurer séparément la coercition répétée du dictionnaire `IsSymbol`, localisée par le profil, si un jalon plus petit est nécessaire ; ne pas lui prêter un gain architectural. Reprofiler le TAST avant de lui transposer le diagnostic JSON. Les 8–9 ms des moteurs manuels restent un potentiel architectural, pas un résultat du compilateur.

**Point 11 toujours ouvert :** la FFI de traversée est allégée, mais le TAST ne gagne que modestement. Reprofiler ce décodeur avant sa prochaine optimisation propre ; `resolveArgs` et les applications à 6–10 arguments sont déjà corrigés. Le parseur natif d'Argonaut est maintenant remplacé par un chemin direct compatible, validé sur JSON/TAST ; ce sous-chantier est clos pour les applications régénérées. Le compilateur natif lui-même devra être reconstruit avec cette FFI avant de mesurer son chargement, puis le vrai b8x complet (point 7). Le prochain gros chantier de parallélisme reste le point 4 : ordonnancement PBO entre modules avec transmission des directives conservée. Défauts natifs : chargement 8, émission 8, préparation 2 ; chargement JS 1.
