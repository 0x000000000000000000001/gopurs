...
_Experimental WIP, but the core ideas have been valided by the facts/results. That will be official soon._
...
3. **Build and Execute:**
   The compiler will parse all `corefn.json` files generated by `purs` and output native Go files in the `output/` directory.
...
The `gopurs` compiler is entirely **zero-config by default**. It will automatically scan your `corefn` ASTs and generate a ready-to-execute `main.go` entrypoint.
