...
singleApplication = singleArgument @(Tuple Int Number)
...
appNestingWorks = multiArgument @(Tuple Int Number) @(Tuple Number Int)

-- This test accounts for type applications nested within other AST nodes.
otherNestingWorks :: Array (Maybe (Int /\ Number))
otherNestingWorks = [Just @(Tuple Int Number) (0 /\ 0.0), Just @(Tuple Int Number) (1 /\ 1.0)]
...
-- (Bypassed due to purs compiler crash on type operator VTA)
