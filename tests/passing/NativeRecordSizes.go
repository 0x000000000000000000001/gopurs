...
var OpaqueValue = gopurs_runtime.Func(func(_ gopurs_runtime.Value) gopurs_runtime.Value {
	return gopurs_runtime.Str("opaque")
})

func NativeShape(record gopurs_runtime.Value) string {
	var keys []string
	shape := ""
	switch record.Type {
...
func NativeMap(record map[string]any) string {
