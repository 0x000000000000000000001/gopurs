...
`expectedGoType = exprTypeToGoType$1_ = unsafePerformEffect (if String.contains (Pattern "__local_var") name then Effect.Console.log ("MakeLet " <> name <> " (" <> modNameStr <> ") -> " <> synName binding <> ", expectedGoType: " <> goTypeToStr expectedGoType) else pure unit)\n          newBound = Map.insert originalName { name, goType: expectedGoType } bound`);
